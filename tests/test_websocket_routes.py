...
class TestRunPipelineSuccessPath:
    ...
    def test_success_path_emits_all_events_in_order(self):
        sent = []
        with patch("websocket_routes.ws_manager.send_json",
                   side_effect=lambda sid, p: sent.append(p["type"])), \
             patch("websocket_routes.execute_admin_query",
                   return_value=self._make_pipeline_result()):
            import websocket_routes
            websocket_routes._run_pipeline(
                "sid1", "show attendance", {}, "trace-001"
            )
...
class TestProgressCallback:
    def test_progress_stages_are_emitted(self):
        ...
        with patch("websocket_routes.ws_manager.send_json",
            ...
            def fake_pipeline(user_query, body, progress_callback, 
                               trace_id):
                for stage in ["planner","intent","dotnet",
                              "combiner","report"]:
