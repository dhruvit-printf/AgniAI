...
def test_concurrent_semantic_catalog_build_loads_model_once():
    ...
    if sc._build_thread.is_alive():
        sc._build_thread.join(timeout=5)
    ...
    # Also directly patch load_embedding_model in rag module to trace calls
    _orig_load = rag.load_embedding_model

    def traced_load():
        print(f"  [TRACE] load_embedding_model called, _MODEL={rag._MODEL is not None}")
        result = _orig_load()
        print(f"  [TRACE] load_embedding_model done, _MODEL={rag._MODEL is not None}")
        return result

    rag.load_embedding_model = traced_load
