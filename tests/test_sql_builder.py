...
def test_sql_builder_is_null_interceptor():
    ...
    ast.columns = ["AgniveerMaster.Id"]
    ast.where.append(WhereNode(column="AgniveerMaster.Remarks", operator="=", value=None))
...
def test_sql_builder_having_clause():
    ...
    ast.having = [WhereNode(column="TotalCount", operator=">", value=5)] # Having filters on alias usually or aggregate
    
    sql, params = sql_builder.build(ast)
    expected = "SELECT COUNT(t0.Id) AS TotalCount FROM AgniveerMaster t0 HAVING TotalCount > ?"
    assert sql == expected
    assert params == [5]
...
def test_sql_builder_safe_aggregate_aliasing():
    ...
    ast.aggregates = [AggregateNode(column="CompanyMaster.Id", function="COUNT", alias="Total")]
