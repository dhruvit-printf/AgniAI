...
def test_operator_sql_injection_is_blocked():
    ...
    ast.where.append(
        WhereNode(
            column="AgniveerMaster.Id", operator="= 1; DROP TABLE Users --", value=1
        )
    )
...
def test_cartesian_join_is_blocked():
    ...
    ast.joins.append(
        JoinNode(
            left_table="AgniveerMaster",
            right_table="PlatoonMaster",
            left_column="PlatoonId",
            right_column="Id",
        )
    )
    ...
    ast.joins.append(
        JoinNode(
            left_table="CompanyMaster",
            right_table="DistributionMaster",
            left_column="Id",
            right_column="CompanyId",
        )
    )
...
def test_duplicate_aggregate_alias_is_blocked():
    ...
    ast.aggregates = [
        ...
        AggregateNode(
            column="AgniveerMaster.PlatoonId", function="COUNT", alias="Total"
        ),
    ]
