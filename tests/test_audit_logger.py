...
class TestAuditLogSchema:
    def test_audit_log_to_dict_has_required_fields(self):
        ...
        d = entry.to_dict()
        for field in ("timestamp", "trace_id", "session_id", 
                      "query_type", "query_duration", "success"):
            assert field in d, f"Missing field: {field}"

    def test_audit_log_timestamp_is_iso_format(self):
        from audit_logger import AuditLog
        entry = AuditLog(
            trace_id="t", session_id="s", query_type="q",
            query_duration=1.0, success=True
        )
    ...
    def test_audit_log_to_json_is_valid_json(self):
        from audit_logger import AuditLog
        entry = AuditLog(
            trace_id="t", session_id="s", query_type="simple",
            query_duration=50.0, success=True
        )
    ...
    def test_audit_log_error_type_stored(self):
        from audit_logger import AuditLog
        entry = AuditLog(
            trace_id="t", session_id="s", query_type="error",
            query_duration=10.0, success=False, 
    ...
    def test_audit_log_none_error_type_when_success(self):
        ...
        entry = AuditLog(
            trace_id="t", session_id="s", query_type="simple",
            query_duration=10.0, success=True
        )
    ...
    def test_audit_log_truncates_long_trace_id(self):
        ...
        long_id = "x" * 200
        entry = AuditLog(
            trace_id=long_id, session_id="s", query_type="q",
            query_duration=1.0, success=True
        )
    ...
    def test_audit_log_rounds_duration(self):
        ...
        entry = AuditLog(
            trace_id="t", session_id="s", query_type="q",
            query_duration=123.456789, success=True
        )
...
class TestWriteAuditLog:
    ...
    def test_write_audit_log_skipped_when_flag_disabled(self):
        ...
        with patch("audit_logger._get_audit_logger") as mock_get_logger, \
             patch("feature_flags.flags") as mock_flags:
    ...
    def test_write_audit_log_includes_error_type(self):
        with patch("audit_logger._get_audit_logger") as mock_get_logger:
            ...
            write_audit_log(
                trace_id="t", session_id="s", query_type="error",
                query_duration=50.0, success=False,
