...
def generate_report():
    md_schema = parse_md_schema(r"C:\Users\dhruv\.gemini\antigravity-ide\brain\d6a094fd-1341-4992-9ec1-f64684561bbb\database_schema.md")
    ...
    for table, md_cols in md_schema.items():
        ...
        actual_cols = {c.replace('[', '').replace(']', '').replace("'", "") for c in actual_cols_info.keys()}
        # but also keep a mapping to the raw name
        raw_col_map = {c.replace('[', '').replace(']', '').replace("'", ""): c for c in actual_cols_info.keys()}
        ...
        for col in missing_in_md:
            ...
            report.append(f"**Current:** Missing column `{raw_col_map[col]}` in SCHEMA_CARD")
