...
def main():
    parser = argparse.ArgumentParser(description="Regenerate actual_schema.json from the live database schema.")
    parser.add_argument("--write", action="store_true", help="Write changes to actual_schema.json and save a backup.")
    args = parser.parse_args()

    schema_json_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../actual_schema.json"))
    ...
    tables_to_add = sorted(list(live_tables - original_tables - {"__EFMigrationsHistory"} - denied_tables))
