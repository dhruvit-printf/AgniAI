...
def verify_step_3(num_threads=5, stress_runs=3):
    print("=" * 60)
    print(f"STEP 3: Concurrent requests on clean cold state ({num_threads} threads, {stress_runs} runs)")
    print("=" * 60)
    ...
    for run_idx in range(1, stress_runs + 1):
        ...
        print(f"Run {run_idx}/{stress_runs}: {num_threads} threads finished in {elapsed:.2f}s | Constructions: {init_counter['n']}")
        assert init_counter["n"] == 1, f"Run {run_idx} EXPECTED 1 construction, GOT {init_counter['n']}"

    print(f"PASSED: Step 3 verified across {stress_runs} runs with {num_threads} threads.\n")
...
def verify_step_4():
    ...
    dt1 = time.time() - t0
    print(f"Call 1 ('Who has viral fever?'): {dt1:.4f}s | Stage 2 hit: {res1 != 'rag'} | Constructions: {init_counter['n']}")

    # Call 2: Needs Stage 2 (First Stage 2 call -> cold load)
    t0 = time.time()
    res2 = classify_intent("Who has cancer?")
    dt2 = time.time() - t0
    print(f"Call 2 ('Who has cancer?'): {dt2:.4f}s | Cold Load Cost Paid | Constructions: {init_counter['n']}")

    # Call 3: Needs Stage 2 (Second Stage 2 call -> cached)
    t0 = time.time()
    res3 = classify_intent("Who has diabetes?")
    dt3 = time.time() - t0
    print(f"Call 3 ('Who has diabetes?'): {dt3:.4f}s | Fast Cached Call | Constructions: {init_counter['n']}")

    assert init_counter["n"] == 1, f"Expected 1 total model construction across sequence, got {init_counter['n']}"
    assert dt3 < dt2, "Cached call (dt3) should be significantly faster than cold load (dt2)"
