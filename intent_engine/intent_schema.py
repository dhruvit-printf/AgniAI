...
OPERATIONS_BY_CATEGORY: Dict[str, FrozenSet[str]] = {
    ...
    "Leave": frozenset([
        "Most", "Least", "Current", "Absconded",
    ]),
    "Medical": frozenset([
        "Active", "BMI", "BloodGroup", "Disease", "Individual",
    ]),
    "Attendance": frozenset([
        "Monthly", "Weekly", "Daily", "Yearly", "Present", "Summary",
    ]),
    "Strength": frozenset([
        "Summary",
    ]),
    ...
    "Distribution": frozenset([
        "Latest", "ByUnit", "Unassigned", "TopUnit",
    ]),
    "Skills": frozenset([
        "BySport", "ByClass",
    ]),
...
DETAILED_KEYWORDS: FrozenSet[str] = frozenset([
    "detail", "detailed", "complete", "full report", "complete report",
    "comprehensive", "everything", "elaborate", "deep analysis",
])
...
CATEGORY_KEYWORDS: Dict[str, Tuple[str, ...]] = {
    "Performance": (
        "performance", "top performer", "top performers", "highest performer",
        "best performer", "bottom performer", "lowest performer", "grading",
        "average score", "attempt", "improvement", "drop", "comparison",
        "section summary", "overall", "pass rate", "fail rate",
        "pass percentage", "fail percentage",
    ),
    "Leave": (
        "leave", "absent", "absconded", "on leave", "days off", "off days",
        "leave taken", "leave status",
    ),
    "Medical": (
        "medical", "health", "bmi", "blood", "weight", "fitness", "disease",
        "active cases", "admitted", "hospital", "sick", "fever", "injury",
    ),
    "Attendance": (
        "attendance", "present", "campus", "muster", "strength", "headcount",
        "monthly", "weekly", "daily", "yearly",
    ),
    "Verification": (
        "verification", "verify", "verified", "pending", "rejected", "approved",
        "cleared",
    ),
    "Equipment": (
        "equipment", "issued", "procured", "overdue", "holding", "condition",
        "gear", "kit", "inventory",
    ),
    "Distribution": (
        "distribution", "unit", "assigned", "agniveer assignment",
    ),
    "Roster": (
        "roster", "rosters", "sport roster", "class roster", "sports roster",
    ),
    "Strength": (
        "strength", "headcount", "breakdown", "strength breakdown",
    ),
    "Skills": (
        "skills", "sport", "cricket", "football", "class", "community",
    ),
    "Schedule": (
        "schedule", "training", "agenda",
    ),
    "Overall": (
        "overall", "composite", "all criteria",
    ),
}
...
OPERATION_SYNONYMS: Dict[str, Dict[str, Tuple[str, ...]]] = {
    "Performance": {
        ...
        "Bottom": (
            "bottom", "lowest", "worst", "bottom performer", "lowest performer",
            "worst performer", "poor performer", "weakest performer",
            "trailing performer", "struggling performer", "underperformer",
            "low scorer", "who scored lowest", "who performed worst",
        ),
        "Improvement": (
            "improvement", "improve", "improved", "score went up", "positive trend",
            "biggest improvement", "most improved", "rising score", "upward trend",
            "who improved", "who got better", "score gain", "bounced back",
            "getting better", "progressing",
        ),
        "Drop": (
            "drop", "decline", "declined", "score drop", "downward trend",
            "negative trend", "who dropped", "who declined", "who got worse",
            "fell", "slipped", "performance degraded", "regression", "regressed",
            "deterioration", "who is struggling more",
        ),
        "Grading": (
            "grading", "grade", "by grade", "grade wise", "who got excellent",
            "who got good", "who got sat", "categorised by grade",
        ),
        "GradingSummary": (
            "grading summary", "grade summary", "distribution of grades",
            "how many in each grade", "grade count", "grade statistics",
        ),
        "Average": (
            "average", "avg", "mean", "average score", "average marks",
            "what is the average", "overall average",
        ),
        "AttemptWise": (
            "attemptwise", "attempt wise", "by attempt", "attempt breakdown",
            "score per attempt", "each attempt", "all attempts", "multiple attempts",
        ),
        "BestAttempt": (
        ...
        "Summary": (
            ...
            "snapshot", "brief", "report",
        ),
        "PassPercentage": (
            "pass percentage", "pass rate", "passing percentage",
            "how many passed", "who passed", "clearance rate",
        ),
        "FailPercentage": (
            "fail percentage", "fail rate", "failure rate",
            "how many failed", "who failed",
        ),
        "Overall": (
            "overall", "composite", "all criteria", "all section",
            "combined performance", "aggregate",
        ),
    },
    "Leave": {
        "Most": (
        ...
        "least", "lowest", "minimum", "least leave taken",
            "who has the least leave", "fewest days absent", "fewest leaves",
            "most regular", "most punctual",
        ),
        "Current": (
            "current", "today", "now", "currently on leave",
            "who is on leave", "leave status", "leave today",
            "absent today", "currently absent",
        ),
        "Absconded": (
            "absconded", "abscond", "gone missing", "missing person",
            "awol", "whereabouts unknown",
        ),
    },
    "Medical": {
        "Active": (
            "active", "cases", "admitted", "active medical cases",
            "current patients", "in hospital", "under treatment",
            "how many sick", "who is admitted",
        ),
        "BMI": (
            "bmi", "weight", "fitness", "body mass index",
            "who is obese", "overweight", "underweight",
        ),
        "BloodGroup": (
            "blood", "blood group", "blood type", "blood group distribution",
            "who has which blood group",
        ),
        "Disease": (
            "disease", "diseases", "diagnoses", "top diseases",
            "common disease", "most common disease",
        ),
        "Individual": (
            "individual medical", "medical status of", "medical details of",
            "particular medical", "specific medical",
        ),
    },
    "Attendance": {
        "Monthly": (
            "monthly", "month", "monthly attendance", "attendance this month",
            "month wise", "attendance by month",
        ),
        "Weekly": (
            "weekly", "week", "weekly attendance", "this week",
            "week wise", "attendance by week",
        ),
        "Daily": (
            "daily", "day", "daily attendance", "day wise",
        ),
        "Yearly": (
            "yearly", "year", "yearly attendance", "annual",
        ),
        "Present": (
            "present", "present today", "on campus", "who is present",
            "how many present", "attendance today", "today attendance",
        ),
        "Summary": (
            "attendance summary", "attendance overview", "summary of attendance",
        ),
    },
    "Equipment": {
        "Stats": (
            "stats", "summary", "overview", "equipment stats",
    ...
    "Distribution": {
        "Latest": (
            "latest", "recent", "last", "newest", "most recent",
            "current", "fresh",
        ),
        "ByUnit": (
            "by unit", "unit wise", "in unit", "agniveers in unit",
            "unit distribution",
        ),
        "Unassigned": (
            "unassigned", "not assigned", "no unit", "without unit",
        ),
        "TopUnit": (
    ...
    "Roster": {
        "BySport": (
            "by sport", "sport wise", "cricket players", "football players",
            "who plays", "sports roster",
        ),
        "ByClass": (
    ...
    "Verification": {
        "Pending": (
            "pending", "awaiting", "not verified", "verification pending",
        ),
        "Sent": (
            "sent", "sent verification", "verification sent",
        ),
        "NotResponded": (
    ...
    "Schedule": {
        "Company": (
            "company schedule", "schedule by company", "company wise schedule",
        ),
        "Agniveer": (
            "agniveer schedule", "schedule by agniveer", "agniveer wise schedule",
        ),
        "Date": (
            "date schedule", "schedule by date", "date wise schedule",
        ),
    },
    "Overall": {
        "OverallPerformance": (
            "overall performance", "overall report", "composite report",
            "all criteria", "all criteria performance",
        ),
    },
}
...
SUBSECTIONS_BY_SECTION: Dict[str, List[str]] = {
    ...
    "Firing": ["300m 5 FTS", "200m 5 FTS", "100m LU (R)", "100m LU (5/5)", "50m SU TRU", "100m NI SFTS SS", "15m (Bal) BC"],
    "Drill": ["Turn Out and Bearing", "Word of Command", "Marching", "Slow March", "Saluting Salami Shastra", "Turning"],
}
...
ISSUED_EQUIPMENT_ITEMS: List[str] = [
    ...
    "Short KD", "Knee & Elbow", "Socks Woollen OG",
    "Trouser Drill Khaki Poly", "Shirt Man Khaki Poly", "Short KD Light Green",
    "Jersey Man Woollen OG", "Shirt Angola Drive", "Trouser Bd Serge",
]

PROCURED_EQUIPMENT_ITEMS: List[str] = [
    "Rect Bag (Khaki)", "Pt Shoes", "Mufti Shoes", "Black Socks",
    "Black Pagri with Fifty", "Drill Shoes", "PT Dress Complete", "Games Dress",
    "Mufti Dress (White)", "Regt Tie with Clip", "Mufti Dress (Winter) / Mufti Blazer",
    "Khaki Half Sleeves (Drill)", "Khaki Full Sleeves (WT & Drill)", "Socks OG",
    "Green Pagri with Fifty", "OG Dress", "Combat Cap", "Leather Belt with Crest",
    "Bed Sheet", "Rect Shoulder", "Name Plate (Black)", "Name Plate (Khaki)",
    "Belt (Black)", "Water Bottle 02 Ltr", "Mug Plastic", "Box Steel with Paint",
    "Hanger", "Health Card", "Bed Card", "Locker Cloth", "Firing Data Card",
    "Vest", "Track Suit with Name", "Clip Board", "256 Pages Copy", "White Hanky",
