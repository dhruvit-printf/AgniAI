...
ALL_QUESTIONS = [
    ...
    (
        12,
        "Show top 10 BPET performers in Lakhwinder company whose police verification is rejected",
    ),
    (13, "Agniveers in Jaswant company who play volleyball and are on medical leave"),
    ...
    (
        16,
        "Agniveers in Lakhwinder company who failed firing and still have issued equipment",
    ),
    (17, "Top BPET scorers in Jaswant company who are on medical leave"),
    (
        18,
        "Top drill performers in Mahadev company whose medical status is hospitalized",
    ),
    (
        19,
        "Agniveers in Krishna company whose police verification is pending and are absent",
    ),
...
def main():
    ...
    print(
        f"Difference: {new_tables - old_tables} tables, {new_cols - old_cols} columns."
    )
    ...
    engine_cols = sum(
        len(schema_engine.get_columns(t)) for t in schema_engine.get_tables()
    )
    print(
        f"SchemaEngine current state: {engine_tables} active tables, {engine_cols} total columns."
    )
    if engine_tables == new_tables:
        print(
            "SUCCESS: SchemaEngine has successfully reloaded the new actual_schema.json."
        )
    else:
        print(
            "WARNING: SchemaEngine table count does not match actual_schema.json. Caches might need clearing."
        )
    ...
    print("\nBATCH SUMMARY TABLE:")
    print(
        f"{'Question':<8} | {'Query Type':<20} | {'Legs':<5} | {'Tier 2 Fallback?':<16} | {'Status/Error':<60}"
    )
    print("-" * 120)
    ...
    for r in batch_results:
        ...
        print(
            f"{q_label:<8} | {q_type:<20} | {legs_count:<5} | {tier2:<16} | {status:<60}"
        )
