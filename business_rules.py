...
R10. BMI / MEDICAL: BMI is computed PER AGNIVEER, never as a cross-agniveer average. For each agniveer, use their own MOST RECENT `MedicalRecordMaster` row (`ORDER BY VisitDate DESC`) where Height AND Weight are both present; if that agniveer has no such row, fall back to `AgniveerMaster.Height`/`AgniveerMaster.Weight`. Formula (Height in cm, Weight in kg): `BMI = Weight / POWER(Height / 100.0, 2)`. Category thresholds: BMI < 18.5 = Underweight; 18.5 <= BMI < 25.0 = Normal; 25.0 <= BMI < 30.0 = Overweight; BMI >= 30.0 = Obese. When the question asks which agniveer(s) fall in a BMI category, compute each agniveer's BMI first, then filter/bucket by these thresholds.
...
R18. SECTIONS AND SUBSECTIONS: The valid CORE sections (ScoreSectionMaster.IsExceptional = 0) are BPET, PPT, Firing, Drill. The valid EXCEPTIONAL sections (IsExceptional = 1 — see R3 for how their marks are computed) are Map Reading, Tactics, Exceptional Sport, BFC, PDP, MR, IT, FC/BC, CBRN, Test Written (AMT), Test Prac (AMT). "Theory" is NOT a real section — do not use it. Do not hallucinate any other section name. BPET includes 5km, 60m Sprint, 9' Ditch, H Rope, V Rope. PPT includes 2.4km, 100m Sprint, Chin Ups, Sit Ups, Toe Touch, 5m Shuttle. Firing includes 300m 5 FTS, 200m 5 FTS, 100m LU (R), 100m LU (5/5), 50m SU TRU, 100m NI SFTS SS, 15m (Bal) BC. Drill includes Turn Out and Bearing, Word of Command, Marching, Slow March, Saluting Salami Shastra, Turning.
""".strip()
