...
R9. ABSCONDED LEAVE: Absconded is detected via `IsAbscondedLeave = 1` directly in `AgniveerLeaveMaster`.
...
R11. VERIFICATION: You MUST use a Window Function (`ROW_NUMBER() OVER (PARTITION BY AgniveerId ORDER BY SentDate DESC)`) on `PoliceVerificationMaster` to find the latest verification status. `Pending` is when there is no record, `NotResponded` is when `Status = 'Sent'` and `ReceivedDate IS NULL`.
