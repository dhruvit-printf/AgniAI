...
def main():
    ...
    print(f"{'Table Name':<35} | {'Missing From DB (Drifted)':<35} | {'Missing From JSON (New)':<35}")
    print("-" * 111)
    ...
    for idx, sql_text in enumerate(sql_lines):
        ...
        alias_matches = re.finditer(r'\b([a-zA-Z_]\w*)\s+(?:AS\s+)?([a-z])\b', sql_text, re.IGNORECASE)
        alias_map = {}
        ...
        col_refs = re.findall(r'\b([a-z0-9_]+)\.([a-zA-Z_]\w*)\b', sql_text, re.IGNORECASE)
            ...
            if alias.lower() in alias_map:
                ...
                if resolved_table in live_schema:
                    ...
                    if col.lower() not in live_cols_lower:
                        print(f"  [ALERT] Column '{resolved_table}.{col}' referenced but missing in live DB!")
    ...
    print(f"extracted_schema.sql declares 'DisqualifiedDate' in tables: {declared_tables_for_dq}")
